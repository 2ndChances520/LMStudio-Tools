...
![Model](https://img.shields.io/badge/Model-Gemma_4-orange?style=for-the-badge)
![Browser](https://img.shields.io/badge/Browser-Brave-red?style=for-the-badge&logo=brave)
...
1. Open the `.json` tool configuration file provided in this repo.
2. Find the `"username": "YOUR_USERNAME_HERE"` field.
3. **Add your username** to replace the placeholder and save the file.

### Step 3: Import Configs to LM Studio

Now, let's inject the tools into LM Studio:

1. Navigate to the **Developer / Tools** section in LM Studio.
2. **Upload the `.json` file** you just edited. This defines the tool structures (like the Brave browser integration).
3. Copy the **System Prompt** from this repo (`system_prompt.txt`) and paste it into the System Prompt configuration box in LM Studio. *(This tells Gemma 4 how and when to use the tools).*
